	...
	if(param && (param.includes('jiosaavn.com/song/') || param.includes('saavn.com/song/'))) {
		...
		if(get_data.result === 'false') {
			...
			get_data.forEach(song => {
				txt += `<tr><td><a href='/?song=${song.url}'>${song.title} By ${song.more_info.primary_artists}</a></td><td><a href='/album/?id="+t.album_id+">"+t.album+"</a></td></tr>`
			});
			txt += "</tbody></table>"
